...
# focused_monitor=$(niri msg outputs | sed -n 's/.*(\(HDMI[^)]*\)).*/\1/p')

# Get monitor width and DPI
monitor_width=$(hyprctl monitors -j | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .width') # For Hyprland
scale_factor=$(hyprctl monitors -j | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .scale') # For Hyprland
# monitor_width=$(niri msg outputs | awk '/Logical size:/ {split($3,a,"x"); print a[1]}') # For Niri
# scale_factor=$(niri msg outputs | awk '/Scale:/ {print $2}') # For Niri

# Calculate icon size
if [[ "$monitor_width" =~ ^[0-9]+$ && "$scale_factor" =~ ^[0-9.]+$ ]]; then
